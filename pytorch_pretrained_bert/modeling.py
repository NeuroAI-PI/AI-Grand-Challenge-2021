...
class BertForSequenceClassificationMLP1(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        x = self.dropout(x)
        x = self.dense(x)
        x = gelu(x)  # although BERT uses tanh here, it seems Electra authors used gelu here
        x = self.dropout(x)
...
class BertForSequenceClassificationCNN(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2):
        ...
        self.static = static
        self.convs1 = nn.ModuleList([nn.Conv2d(1, 1, (K, config.hidden_size)) for K in self.kernel_sizes])
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.fc1 = nn.Linear(437, 128)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        x = [F.max_pool1d(i, 3).squeeze(1) for i in x]  # [(N, Co), ...]*len(Ks)
        x = torch.cat(x, 1)
        x = self.dropout(x)  # (N, len(Ks)*Co)
        x = self.fc1(x)  # (N, C)
        logits = self.fc2(x)  # (N, C)
